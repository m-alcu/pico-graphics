...
// 16 for ST7789 or 32 for ST7735, 240 with pico_graphics
#define NLINES 240
...
int fastSin(int i)
{
  while(i < 0) i += 360;
  while(i >= 360) i -= 360;
  if(i < 90)      return sinTab[i];
  else if(i < 180) return sinTab[180 - i];
  else if(i < 270) return -sinTab[i - 180];
  else             return -sinTab[360 - i];
}
...
void drawQuad( int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, uint16_t c, uint16_t* frBuf) 
{
  int x,y;
...
void drawTri( int x0, int y0, int x1, int y1, int x2, int y2, uint16_t c, uint16_t* frBuf) 
{
  int x,y;
  int16_t line[NLINES*2];
  for(y=0;y<NLINES;y++) { line[2*y+0] = WD_3D+1; line[2*y+1] = -1; }
...
void drawQuads(int16_t *v2d, uint16_t* frBuf) 
{
...
void drawTris(int16_t *v2d,  uint16_t* frBuf) 
{
  int q,v0,v1,v2,v3,c,i;
  for(i=0;i<numVisible;i++) {
    q = sortedPolys[i];
    ...
    if(lightShade>0) {
      c = normZ[q]*255/18000;
      if(c>255) c=255;
      drawTri(v2d[2*v0+0],v2d[2*v0+1],  v2d[2*v1+0],v2d[2*v1+1], v2d[2*v2+0],v2d[2*v2+1], pimoroni::RGB(c,c,c/2).to_rgb565(), frBuf);
    } else
      drawTri(v2d[2*v0+0],v2d[2*v0+1],  v2d[2*v1+0],v2d[2*v1+1], v2d[2*v2+0],v2d[2*v2+1], polyColors[q], frBuf);
  }
} 

// animated checkerboard pattern
void backgroundChecker(int i, uint16_t* frBuf)
{
  int x,y,xx,yy, xo,yo;
  ...
  for(y=0;y<NLINES;y++) {
    ...
    for(x=0;x<WD_3D;x++) {
      xx = (x+xo) % 64;
      frBuf[SCR_WD*y+x] = ((xx<32 && yy<32) || (xx>32 && yy>32)) ? pimoroni::RGB(40,40,20).to_rgb565() : pimoroni::RGB(80,80,40).to_rgb565();
    }
  }
}

void backgroundPattern(int i, const unsigned short *pat, uint16_t* frBuf)
{
  int x,y,xp,yp;
...
void initStar(int i)
{
  stars[i].x = randRange(-500, 500);
  stars[i].y = randRange(-500, 500);
  stars[i].z = randRange(100, 2000);
...
void backgroundStars(int f, uint16_t* frBuf)
{
  int i;
  ...
  for(i=0; i<NUM_STARS; i++) {
    ...
    if(r>255) r=255;
    if(r<40) r=40;
    uint16_t col = pimoroni::RGB(r, r, r).to_rgb565();
    int x = stars[i].x2d;
...
void render3D(int mode=0, uint16_t* frBuf)
{
  ...
  for(i=0;i<HT_3D;i+=NLINES) {
    yFr = i;
    if(bgMode==0) backgroundPattern(t,pat2, frBuf); else
    if(bgMode==1) backgroundPattern(t,pat8, frBuf); else
    if(bgMode==2) backgroundPattern(t,pat7, frBuf); else
    if(bgMode==3) backgroundStars(t, frBuf); else
    if(bgMode==4) backgroundChecker(t, frBuf);
    mode ? drawTris(projVerts, frBuf) : drawQuads(projVerts, frBuf);
